...
export const EditResourceModal = () => {
  ...
  useEffect(() => {
    if (data.resourcePatternName) {
      form.setValue("name", data.resourcePatternName);
    }
    if (data.costPricePerUnit) {
      form.setValue("costPricePerUnit", data.costPricePerUnit);
    }
    if (data.orderPricePerUnit) {
      form.setValue("orderPricePerUnit", data.orderPricePerUnit);
    }
    if (data.measureUnit) {
  ...
  }, [form, data.resourcePatternId, data.resourcePatternName, data.costPricePerUnit, data.orderPricePerUnit, data.measureUnit, data.resourceType, isOpen]);

  const isLoading = form.formState.isSubmitting;

  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    const response = await updateResource({
      resourcePatternId: data.resourcePatternId!,
    ...
    const dataForEditResource = {
      resourcePatternId: data.resourcePatternId!,
      resourcePatternName: response.data.name!,
      costPricePerUnit: response.data.costPricePerUnit,
      orderPricePerUnit: response.data.orderPricePerUnit,
      measureUnit: response.data.measureUnit,
      resourceType: response.data.resourceType,
    };
    dispatch(editResource(dataForEditResource));
